...
module("Unit | Component | freestyle dynamic", function(hooks) {
  ...
  test("dynamicProperties is initialized to {} if value passed in is falsey", function(assert) {
    ...
    let component = this.owner.factoryFor('component:freestyle-dynamic').create({ dynamicProperties: null });
    assert.deepEqual(component.dynamicProperties, {});

    component = this.owner.factoryFor('component:freestyle-dynamic').create({ dynamicProperties: undefined });
    assert.deepEqual(component.dynamicProperties, {});
  });
});
